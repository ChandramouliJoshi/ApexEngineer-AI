...
export default function Laps() {
  ...
  const { laps, loading, error } = useLaps({
    year,
    grandPrix,
    driver,
    sessionType,
  ...
  const fastestLap = useMemo(() => {
    ...
    return validLaps.reduce((fastest, lap) =>
      ...
      (lap.LapTime !== null && lap.LapTime < fastest.LapTime)
        ? lap
        : fastest
    )
  }, [validLaps])
  ...
  const averageLap = useMemo(() => {
    if (!validLaps.length) return null
    return (
      validLaps.reduce((sum, lap) => sum + (lap.LapTime ?? 0), 0) /
      validLaps.length
    )
  }, [validLaps])
  ...
  const topSpeed = useMemo(() => {
    const speeds = validLaps
      .flatMap((lap) => [lap.SpeedFL, lap.SpeedST])
  ...
  const bestSectors = useMemo(() => {
    ...
    return {
      s1: best(validLaps.map((lap) => lap.Sector1Time)),
      s2: best(validLaps.map((lap) => lap.Sector2Time)),
      s3: best(validLaps.map((lap) => lap.Sector3Time)),
    }
  }, [validLaps])
  ...
  const consistency = useMemo(() => {
    ...
    const variance =
      times.reduce((sum, v) => sum + Math.pow(v - mean, 2), 0) /
      times.length
    const sd = Math.sqrt(variance)

    return Math.max(0, Math.min(100, 100 - (sd / mean) * 1000))
  }, [validLaps])
  ...
  const paceChange = useMemo(() => {
    ...
    const avg = (group: typeof ordered) =>
      group.length
        ? group.reduce((sum, lap) => sum + (lap.LapTime ?? 0), 0) /
          group.length
        : null
    ...
    return firstAvg !== null && secondAvg !== null
      ? secondAvg - firstAvg
      : null
  }, [validLaps])
  ...
  const stints = useMemo(() => {
    ...
    groups.forEach((group) => {
      const stintLaps = validLaps.filter(
        (lap) => (lap.Stint ?? 0) === group.stint
      )
      group.average =
        stintLaps.reduce((sum, lap) => sum + (lap.LapTime ?? 0), 0) /
        stintLaps.length
    })

    return Array.from(groups.values()).sort((a, b) => a.stint - b.stint)
  }, [validLaps])
  ...
  const chartData = useMemo(() => {
    ...
    const points = progression.map((lap, index) => {
      const value = lap.LapTime ?? rawMax
      const x =
        paddingLeft +
        (index / Math.max(progression.length - 1, 1)) * plotWidth
      const normalized = (value - min) / Math.max(max - min, 0.001)
      const y = paddingTop + normalized * plotHeight

      return { x, y, lap, value }
    })

    const average = averageLap ?? rawMin
    const averageY =
      paddingTop +
      ((average - min) / Math.max(max - min, 0.001)) * plotHeight

    const fastestIndex = progression.findIndex(
      (lap) => lap.LapNumber === fastestLap?.LapNumber
    )
    ...
    return {
      ...
      fastestPoint:
        fastestIndex >= 0
          ? {
              x: points[fastestIndex].x,
              y: points[fastestIndex].y,
              lap: points[fastestIndex].lap,
            }
          : null,
      width,
      height,
  ...
  }, [progression, averageLap, fastestLap])

  const chartPoints =
    chartData?.points.map((point) => `${point.x},${point.y}`).join(" ") ?? ""

  const chartTicks = useMemo(() => {
    if (!chartData?.points.length) return []
    const desired = 7
    const step = Math.max(1, Math.ceil(chartData.points.length / desired))

    return chartData.points
      .filter(
        (_, index) =>
          index % step === 0 || index === chartData.points.length - 1
      )
      .map((point) => ({
        x: point.x,
        label: String(point.lap.LapNumber),
      }))
  }, [chartData])
  ...
  const yTicks = useMemo(() => {
    ...
    return Array.from({ length: count + 1 }, (_, index) => {
      ...
      const value = chartData.max - ratio * (chartData.max - chartData.min)
      const y = chartData.paddingTop + ratio * chartData.plotHeight
      return { y, value }
    })
  }, [chartData])
    ...
    () =>
      hoveredLapNumber === null
        ...
        : chartData?.points.find(
            (point) => point.lap.LapNumber === hoveredLapNumber
          ) ?? null,
    [hoveredLapNumber, chartData]
  )
  ...
  const tyreTone: Record<string, string> = {
    SOFT: "border-red-400/30 bg-red-400/5 text-red-300",
    MEDIUM: "border-yellow-400/30 bg-yellow-400/5 text-yellow-300",
    HARD: "border-slate-500/40 bg-slate-400/5 text-slate-300",
    INTERMEDIATE: "border-emerald-400/30 bg-emerald-400/5 text-emerald-300",
    WET: "border-cyan-400/30 bg-cyan-400/5 text-cyan-300",
  }
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="mt-2 font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-slate-500">
                    {year} · {grandPrix} Grand Prix · {driverNames[driver] ?? driver} ·{" "}
                    {sessionType === "R"
                      ? "Race"
                      : sessionType === "Q"
                        ? "Qualifying"
            ...
            <div className="flex items-center gap-3">
              <label className="relative">
                ...
                <select
                  ...
                  className="appearance-none rounded-lg border border-cyan-400/25 bg-slate-950 px-4 py-3 pr-10 font-mono text-[10px] font-black uppercase tracking-[0.15em] text-slate-200 outline-none transition hover:border-cyan-300/50 focus:border-cyan-300"
                >
                  {Object.entries(driverNames).map(([code, name]) => (
          ...
          <>
            <section className="grid gap-4 md:grid-cols-3">
              {[
                {
                  label: "Fastest Lap",
                  value: formatLapTime(fastestLap?.LapTime ?? null),
                  meta: fastestLap ? `LAP ${fastestLap.LapNumber}` : "NO DATA",
                  icon: Trophy,
                  tone: "cyan",
                },
                {
                ...
                return (
                  ...
                  >
                    <div className={`absolute left-0 top-0 h-full w-[2px] ${tone.line}`} />
                    ...
                    <p className={`mt-5 font-mono text-4xl font-black ${tone.text}`}>
                      {card.value}
                    </p>
            ...
            <section className="lap-hover-card mt-5 overflow-hidden rounded-xl border border-slate-800/90 bg-slate-950/70 shadow-[0_0_40px_rgba(0,0,0,0.15)]">
              <div className="flex flex-col gap-4 border-b border-slate-800/80 px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-4 font-mono text-[8px] font-bold uppercase tracking-widest">
                  <span className="text-cyan-300">{progression.length} SAMPLES</span>
                  <span className="h-3 w-px bg-slate-800" />
                  <span className="text-slate-500">FASTEST L{fastestLap?.LapNumber ?? "—"}</span>
                </div>
              </div>
                    ...
                    >
                      ...
                      {chartData.averageY !== null && (
                        <g>
                          ...
                          />
                          <rect
                            x={chartData.width - 90}
                            y={chartData.averageY - 13}
                            width="66"
                            height="20"
                            rx="4"
                            fill="#020617"
                          ...
                          />
                          <text
                            x={chartData.width - 57}
                            y={chartData.averageY + 1}
                            textAnchor="middle"
                            fill="#f59e0b"
                            fontSize="9"
                            fontWeight="800"
                      ...
                      {chartData.points.map((point) => {
                        const isFastest =
                          point.lap.LapNumber === fastestLap?.LapNumber
                        const isHovered =
                          point.lap.LapNumber === hoveredLapNumber

                        return (
                          <g
                            key={point.lap.LapNumber}
                            onMouseEnter={() =>
                              setHoveredLapNumber(point.lap.LapNumber)
                            }
                            onMouseLeave={() => setHoveredLapNumber(null)}
                      ...
                      {chartData.fastestPoint && (
                        <g pointerEvents="none">
                          <line
                            x1={chartData.fastestPoint.x}
                            y1={chartData.fastestPoint.y - 9}
                            x2={chartData.fastestPoint.x}
                            y2={chartData.fastestPoint.y - 30}
                            stroke="#22d3ee"
                            strokeWidth="1"
                          />
                          <rect
                            x={Math.max(
                              6,
                              Math.min(
                                chartData.fastestPoint.x - 46,
                                chartData.width - chartData.paddingRight - 92
                              )
                            )}
                            y={Math.max(
                              4,
                              chartData.fastestPoint.y - 53
                            )}
                            width="92"
                            height="22"
                            rx="5"
                          ...
                          <text
                            ...
                            fontFamily="monospace"
                          >
                            FASTEST · L{chartData.fastestPoint.lap.LapNumber}
                          </text>
                        </g>
                      )}
                            ...
                            return (
                              <>
                                ...
                                <text
                                  ...
                                  fontFamily="monospace"
                                >
                                  LAP {hoveredPoint.lap.LapNumber}
                                </text>
                                <text
                                  x={tooltipX + 12}
                                  y={tooltipY + 35}
                                  ...
                                  fontFamily="monospace"
                                >
                                  {formatLapTime(hoveredPoint.lap.LapTime)}
                                </text>
                                <text
                                  x={tooltipX + 88}
                                  y={tooltipY + 35}
                                ...
                                <text
                                  ...
                                  fontFamily="monospace"
                                >
                                  S1 {formatSector(hoveredPoint.lap.Sector1Time)} · S2{" "}
                                  {formatSector(hoveredPoint.lap.Sector2Time)} · S3{" "}
                                  {formatSector(hoveredPoint.lap.Sector3Time)}
                                </text>
                              </>
                            )
                          })()}
                ...
                <div className="space-y-3 p-5">
                  {stints.length === 0 ? (
                    ...
                    stints.map((stint) => {
                      ...
                      const paceDelta =
                        stint.average !== null && averageLap !== null
                          ? stint.average - averageLap
                          : null

                      const stintAverages = stints
                        .map((item) => item.average)
                        .filter((value): value is number => value !== null)

                      const minStintTime = stintAverages.length
                        ? Math.min(...stintAverages)
                        : null
                      const maxStintTime = stintAverages.length
                        ? Math.max(...stintAverages)
                        : null
                      ...
                      const paceWidth =
                        ...
                        minStintTime === null ||
                        maxStintTime === null ||
                        maxStintTime === minStintTime
                          ? 50
                          : 25 +
                            ((maxStintTime - stint.average) /
                              (maxStintTime - minStintTime)) *
                              75
                          ...
                          <div className="mt-4 rounded-md border border-slate-800/80 bg-slate-950/70 p-3">
                            <div className="flex items-center justify-between">
                              ...
                              >
                                {paceDelta === null
                                  ? "—"
                                  : `${paceDelta > 0 ? "+" : ""}${paceDelta.toFixed(3)}s`}
                              </span>
                            </div>
                ...
                <div className="grid gap-3 p-5 sm:grid-cols-2">
                  {[
                          ...
                          {[
                            ...
                            <div key={name as string} className="flex justify-between">
                              ...
                              <span className="font-mono text-[10px] font-black text-cyan-300">
                                {formatSector(value as number | null)}
                              </span>
                            </div>
                          ))}
                        </div>
                      ),
                      className: "border-cyan-400/15 bg-cyan-400/[0.025]",
                    ...
                    },
                    {
                      ...
                      className: "border-emerald-400/15 bg-emerald-400/[0.025]",
                    ...
                    },
                    {
                      ...
                      value: (
                        <>
                          <p className="font-mono text-3xl font-black text-orange-300">
                            {paceChange !== null
                              ? `${paceChange >= 0 ? "+" : ""}${paceChange.toFixed(3)}s`
                              : "—"}
                          </p>
                          <p className="mt-1 font-mono text-[7px] font-bold uppercase tracking-widest text-slate-500">
                      ...
                      className: "border-orange-400/15 bg-orange-400/[0.025]",
                    ...
                    },
                    {
                        ...
                        <>
                          <p className="font-mono text-3xl font-black text-red-300">
                            {topSpeed !== null
                              ...
                              : "—"}
                            {topSpeed !== null && (
                              <span className="ml-2 text-base">KM/H</span>
                            )}
                          </p>
                          <p className="mt-1 font-mono text-[7px] font-bold uppercase tracking-widest text-slate-500">
                            MAXIMUM RECORDED
                      ...
                      className: "border-red-400/15 bg-red-400/[0.025]",
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="lap-metric mt-5">{item.value}</div>
                      </div>
                    )
                  })}
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[1050px] border-collapse">
                  ...
                  <tbody>
                    {laps.map((lap) => {
                      const isFastest =
                        fastestLap?.LapNumber === lap.LapNumber
                            ...
                            <div className="flex items-center gap-3">
                              {isFastest ? (
                                <span className="relative flex h-5 w-5 items-center justify-center rounded-full border border-cyan-300/60 bg-cyan-300/10 shadow-[0_0_14px_rgba(34,211,238,0.28)]">
                                  <Trophy size={9} className="text-cyan-200" />
                                </span>
                              ) : (
                              ...
                              <div className="flex items-center gap-2">
                                <span className={`font-mono text-[10px] font-black ${
                                  isFastest ? "text-cyan-100" : "text-slate-200"
                                }`}>
                                  {lap.LapNumber}
                                </span>
                                {isFastest && (
                          ...
                          <td className="px-5 py-4">
                            <span
                              className={`font-mono text-[10px] font-black ${
                                isFastest ? "text-cyan-200" : "text-cyan-300"
                              }`}
                            >
